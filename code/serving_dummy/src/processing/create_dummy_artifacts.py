# src/processing/create_dummy_artifacts.py
import numpy as np
import faiss
import pickle
...
# --- Configuration ---
# Use a small, standard model for quick testing
...
# Link metadata by filename for easy lookup later
...
# Use a simple Flat L2 index for small dummy data
index = faiss.IndexFlatL2(embedding_dimension)
print(f"Created FAISS IndexFlatL2 with dimension {embedding_dimension}")

# Create mapping from FAISS index ID (0, 1, 2...) to our metadata info
faiss_id_to_info = []
for i, meta in enumerate(dummy_metadata_list):
     faiss_id_to_info.append({
         'source_filename': meta['source_pdf_filename'],
         'chunk_id': meta['chunk_id'] # Or add actual chunk text if needed
     })
     # Add the corresponding embedding to the index
     index.add(np.array([embeddings[i]])) # Add one by one
