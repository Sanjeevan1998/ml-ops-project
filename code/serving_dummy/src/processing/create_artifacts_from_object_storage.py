...
# This path is constructed to be at serving_dummy/generated_artifacts_from_os/faissIndex/v1/
# Assumes this script is in serving_dummy/src/processing/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '../../')) # Goes up two levels from src/processing to serving_dummy
...
# Control for number of files to process
# Set to None to process all files in PDF_SOURCE_DIR_ON_VM
# Set to an integer (e.g., 10000) to process only that many files
MAX_FILES_TO_PROCESS = 5000  # For testing; change to a larger number or None for full processing
...
for i, filename in enumerate(pdf_files_to_process):
    ...
    try:
        ...
        for chunk_idx, chunk_text in enumerate(doc_chunks):
            chunk_id = f"{filename}_chunk_{chunk_idx:04d}" # Unique ID for each chunk
            all_chunks_data.append({
                'source_filename': filename, # Original PDF filename
                'chunk_id': chunk_id,        # Unique ID for this chunk
                'chunk_text': chunk_text     # The actual text of the chunk
            })

        # Create placeholder metadata for this document
        placeholder_metadata_store[filename] = {
            "case_name": f"Case Name for {os.path.splitext(filename)[0]}", # Basic name from filename
            "citation": f"Citation {i+1}",
            "decision_date": f"2024-01-{i+1:02d}", # Dummy date
            "source_pdf_filename": filename,
            # Path for API to construct download link (relative to its PDF_DATA_DIR)
            "source_pdf_path_in_api": filename,
            "judge": f"Judge {chr(65+(i%26))}", # A, B, C... cycles
            "outcome_summary": f"This is a placeholder summary for the document {filename}. It discusses various legal concepts.",
            "key_legal_issues": [f"Legal Issue {j+1} for file {i+1}" for j in range(np.random.randint(2,5))] # Random number of issues
        }
...
# --- Generate Embeddings ---
print(f"\nLoading embedding model: {MODEL_PATH_ON_VM}...")
try:
    embedder = SentenceTransformer(MODEL_PATH_ON_VM, device='cpu') # Explicitly CPU
except Exception as e:
...
# The faiss_id_to_info list directly maps the FAISS index (0, 1, 2...) to the chunk data
...
# --- Save Artifacts ---
# Use standard filenames as these will be the "real" ones
